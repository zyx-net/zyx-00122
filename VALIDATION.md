...
覆盖场景：草稿保存链路、只读模式、状态串链、刷新不丢数据、导出链路、批次授权台账（预演确认、未授权拦截、交接后回滚、日志追溯、重新导出与重启后复核、统一身份状态层、多用户切换验证

---

## 零、统一身份状态层 验证

**验证目标**：用户身份从角色级提升到具体用户级，所有页面共享同一状态源，刷新/重启后状态保持

### 前置条件
1. 启动本地服务：`npm run dev`
2. 清除浏览器 localStorage 数据（可选，确保干净状态）

### 步骤 0.1：首页用户选择流程
```
1. 访问首页 http://localhost:5173/
2. 点击"巡检员"按钮
3. 显示同角色下所有用户列表（张三、李四、王五、赵六、孙七）
4. 点击"巡检员张三"
5. 进入巡检任务列表页
```
**验证点**：
- ✅ 顶部标题栏右侧显示当前用户："巡检员张三"
- ✅ localStorage 中存在 `zyx-00122-app-state 键，包含 currentUser 信息
- ✅ 页面自动跳转到 `/inspector/tasks`
...
2. 弹出用户菜单，显示：
   ...
   - 退出登录按钮
3. 点击"巡检员李四"
4. 页面自动刷新，当前用户变为"巡检员李四"
```
**验证点**：
- ✅ 顶部标题栏右侧显示："巡检员李四"
- ✅ localStorage 中 currentUser 已更新为李四
- ✅ 所有页面状态保持（不丢任何数据
...
- ✅ 刷新后仍保持在当前页面
- ✅ 顶部标题栏右侧仍显示："巡检员李四"
- ✅ localStorage 中 currentUser 信息保持不变
...
**验证点**：
- ✅ 跳转回首页（显示角色选择）
- ✅ localStorage 中 currentUser 已清空
- ✅ 所有页面数据保持（未删除）
...
## 十二、统一身份 & 权限验证 - 三条核心链路（场景 B：完整链路验证）

**验证目标**：通过真实界面操作，验证三条核心链路：
1. 已授权用户能进详情并看到完整信息
2. 未授权但同角色的用户只能看到脱敏摘要且不能导出或回滚
3. 交接给另一人后原授权人立刻失去回滚权限

### 前置条件
1. 完成场景 A（已创建授权批次：查看人=张三+李四，回滚人=张三，接手人=张三）
2. 确保本地服务运行中
...
**验证点**：
- ✅ 批次卡片显示蓝色"已授权 v1"标签，无 🔒 图标
- ✅ 批次名称完整显示（无打码）
- ✅ 操作人完整显示（无打码）
- ✅ 右侧显示"详情"和"回滚"按钮（均可点击）
- ✅ 点击"详情"按钮进入授权详情页
- ✅ 详情页显示完整信息：
  - 批次名称、创建时间、创建人（完整显示）
  - 查看人列表：巡检员张三、巡检员李四
...
**验证点**：
- ✅ 批次卡片显示灰色背景、🔒 Lock 图标
- ✅ 批次名显示为 `批次导入-202***` 形式（打码）
- ✅ 操作人显示为 `王**` 形式（仅保留首字）
- ✅ 卡片底部显示黄色授权提示："已授权给 巡检员张三、巡检员李四 等"
- ✅ 右侧显示"无权限"文字，**没有**"详情"和"回滚"按钮
- ✅ 点击卡片无任何反应（不能进入详情）
- ✅ 在导入中心查看：
  - "导出全部"按钮对该批次禁用
  - "回滚"按钮对该批次禁用
- ✅ 直接在控制台测试权限函数：
  ```javascript
  // 应返回 false
  useAuthorizationStore.getState().hasPermission(auth, 'view', 'inspector_wangwu')
  useAuthorizationStore.getState().hasPermission(auth, 'rollback', 'inspector_wangwu')
  useAuthorizationStore.getState().hasPermission(auth, 'export', 'inspector_wangwu')
  ```
...
- ✅ 批次卡片显示 🔒 图标（或权限降级提示）
- ✅ "回滚"按钮消失或禁用
- ✅ `hasPermission(auth, 'rollback', 'inspector_zhangsan')` 返回 `false`
- ✅ 仍可查看详情（作为查看人），但不能回滚

**验证点 - 交接后新授权人（李四）**：
- ✅ 批次卡片显示蓝色"已授权 v2"标签（版本号递增）
- ✅ "回滚"按钮正常显示且可点击
- ✅ `hasPermission(auth, 'rollback', 'inspector_lisi')` 返回 `true`
- ✅ 交接人列表显示：巡检员李四
- ✅ 操作时间线新增记录："批次从 巡检员张三 交接给 巡检员李四"
- ✅ 授权快照新增 v2 版本

---

## 十三、批次授权台账 - 未授权拦截流程（场景 C）
